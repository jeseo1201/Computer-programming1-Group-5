...
def get_major_names(df):
    if "개설전공(과)" not in df.columns:
        return []

    majors = set()
    for value in df["개설전공(과)"].dropna():
        major = str(value).strip()
        if major:
            majors.add(major)
    return [major for major in ALLOWED_MAJORS if major in majors]
...
def prepare_generation(df, settings):
    ...
    for _, row in df.iterrows():
        ...
        dept = clean(row.get("개설전공(과)", ""))
        time_text = clean(row.get("요일및교시(강의실)", ""))

        if remark and target_major not in [x.strip() for x in remark.split(",")] and dept != target_major:
            continue
...
class ResultWindow:
    ...
    def init_table(self):
        ...
        for col, day in enumerate(DISPLAY_DAYS, 1):
        ...
        for col in range(len(DISPLAY_DAYS) + 1):
